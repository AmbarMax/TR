<template>
  <div class="list-wrapper mt-30 mb-30" v-if="followers">
    <div class="item" v-for="item in followers" :key="item.id">
      <div class="left-part">
...
export default defineComponent({
  ...
  methods: {
    async getFollowers(page) {
      await api.get('/api/follow/followers?page='+page).then(response => {
        if (response && response.data) {
          store.state.followers = response.data[0].data;
          store.state.followers = response.data[0].total;
          store.state.totalFollowing = response.data.totalFollowing;
          store.state.totalFollowers = response.data[0].total;
          this.followers = response.data[0].data;
    ...
    openNetworkRemoveActionModal(userId, userName) {
      store.state.networkRemoveUnfollowModal.title = 'Remove follower '+userName+'?';
  ...
  mounted(){
    this.getFollowers(this.currentPage);
