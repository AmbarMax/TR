# CLAUDE_PHASE_9D_OPS.md — Phase 9D: Dashboard redesign complete

> **Audience:** Claude Code (CLI). Run from `~/Documents/trophyroom`.
> **Read first:** `CLAUDE.md` + this brief.
> **Scope:** Rediseño completo de `Dashboard.vue` siguiendo el mockup aprobado (`dashboard-v2.html`). 6 secciones: Hero (con mascota T-Rex voxel 3D), Ready to forge, Trophies in progress, Daily missions, Recent activity, Connected platforms. Conectado a APIs reales del proyecto.

---

## Goal

Reemplazar completamente el contenido de `Dashboard.vue` por el nuevo diseño del mockup, manteniendo las conexiones a las APIs reales del proyecto. Al terminar:

1. Hero con mascota T-Rex voxel 3D grande a la derecha, stats del usuario (Badges / Trophies / Achievements / Platforms), 2 CTAs.
2. "Ready to forge" muestra el trophy disponible para forjar con badge grande.
3. "Trophies in progress" muestra thumbnails con progress bars.
4. "Daily missions" terminal con progreso, reset timer, y acciones por misión.
5. "Recent activity" real data de notifications o empty state con mascota.
6. "Connected platforms" strip con 5 plataformas (Discord, Steam, GitHub, Riot, Strava).
7. Terminal footer con versión + session info.

NO duplica el wallet-rail del hero — ese vive solo en el main-header global ya implementado en 9A.

---

## Non-goals

- NO tocar `sidebar.vue`, `main-header.vue`, `App.vue`, `Main.vue`.
- NO tocar backend, APIs, Vuex actions o Vuex state.
- NO tocar legacy components (`achievement-card.vue`, `forge-card.vue`, `validate-card.vue`).
- NO rediseñar otras páginas (Trophy Room, Forge, etc. — son phases separadas).
- NO modificar rutas.
- NO implementar nuevas acciones del usuario (forge trophy, sync, etc.) — solo UI que llama métodos que ya existen.

---

## Assets del brief

Un asset nuevo a colocar:

```
resources/web/images/web/img/mascot/
└── trex-voxel-hero.png     (680×680 PNG transparente, mascota del hero)
```

Max lo entrega junto a este brief. Es el T-Rex voxel 3D confident holding trophy, a 2× resolución (680) para displays retina renderizado a 340px.

---

## Pre-work: entender qué APIs usa el Dashboard actual

**Antes de empezar a escribir código**, Claude Code debe hacer 3 cosas para entender el estado actual:

### 1. Leer el Dashboard.vue actual completo

```bash
cat resources/web/js/pages/Dashboard.vue
```

Anotar:
- Qué APIs llama en `mounted()` o `created()`
- Qué data del store consume
- Qué métodos tiene
- Qué componentes importa

### 2. Verificar endpoints reales en `api.js`

```bash
grep -n "api\." resources/web/js/api/api.js | head -30
grep -rn "api/forge\|api/badges\|api/profile\|api/notification\|api/achievement" resources/web/js/ | grep -v node_modules | head -20
```

Identificar qué endpoints del listado del `CLAUDE.md` están realmente disponibles y qué devuelven.

### 3. Verificar campos reales del store

```bash
grep -n "user\.\|userData\.\|badges\|trophies\|forge" resources/web/js/store/store.js | head -30
```

Identificar qué campos hay realmente en el store para `user`, `balances`, `badges`, etc.

**IMPORTANTE:** Si algún endpoint del brief no existe tal cual, Claude Code debe ajustar al nombre real. Si no existe ningún endpoint para algo (ej. "daily missions"), usar un placeholder mock claramente marcado con un comentario `// TODO: wire to backend when available`.

---

## Steps

### Step 1 — Colocar el sprite del hero

**Acción:**

```bash
ls -la resources/web/images/web/img/mascot/trex-voxel-hero.png
```

Max ya lo colocó manualmente. Si falta, pedirlo antes de continuar.

**Commit:** `feat: add T-Rex voxel hero sprite`

---

### Step 2 — Rediseñar Dashboard.vue

**Archivo:** `resources/web/js/pages/Dashboard.vue`

Reemplazar completo el componente. El diseño usa Options API, SCSS scoped, y conecta a las APIs reales del proyecto.
      ...
      <section class="dash-section">
        ...
        <div class="platforms-strip">
          <router-link
            ...
            :key="platform.key"
            :to="platform.synced ? '/profile' : '/profile'"
            class="platform-cell"
...
export default {
  ...
  data() {
    return {
      ...
      // Platforms — placeholder config, sync counts from userBadges
      // TODO: wire "connected" status from real user.integrations when available
      platformsConfig: [
  ...
  computed: {
    username() {
      return this.$store?.state?.userUsername || this.$store?.state?.user?.username || 'User';
    },
    tagline() {
    ...
    stats() {
      return {
        ...
        trophies: this.userTrophies.length,
        achievements: this.userTrophies.filter(t => t.forged).length || this.userTrophies.length,
    ...
    syncedPlatforms() {
      // Derive from userBadges — count badges per platform
      const counts = {};
      this.userBadges.forEach(b => {
        const key = (b.platform || b.source || '').toLowerCase();
    ...
    platforms() {
      // Merge: synced first, then available
      return [
  ...
  methods: {
    async loadDashboardData() {
      try {
        // Fetch in parallel. Adjust endpoint paths if grep reveals different routes.
        const [badgesRes, forgeRes, notifRes] = await Promise.all([
          api.get('/badges').catch(() => ({ data: [] })),
          api.get('/forge').catch(() => ({ data: [] })),
          api.get('/notification').catch(() => ({ data: [] }))
        ]);

        this.userBadges = Array.isArray(badgesRes.data) ? badgesRes.data : (badgesRes.data?.data || []);

        const allTrophies = Array.isArray(forgeRes.data) ? forgeRes.data : (forgeRes.data?.data || []);
        this.userTrophies = allTrophies;

        // Ready to forge: first trophy where badges_owned >= badges_count AND not yet forged
        this.readyTrophy = allTrophies.find(t =>
          (t.badges_owned >= t.badges_count) && !t.forged
        ) || null;

        // In progress: has badges owned but not complete, excluding the ready one
        this.progressTrophies = allTrophies
          .filter(t =>
            t.id !== this.readyTrophy?.id &&
            (t.badges_owned || 0) > 0 &&
            (t.badges_owned || 0) < t.badges_count &&
            !t.forged
          )
          .slice(0, 4);

        this.notifications = Array.isArray(notifRes.data) ? notifRes.data : (notifRes.data?.data || []);

        // Update mission flags based on real state
    ...
    updateMissionStates() {
      ...
      const hasDiscord = this.userBadges.some(b =>
        (b.platform || b.source || '').toLowerCase() === 'discord'
      );
      const m1 = this.missions.find(m => m.id === 1);
      if (m1) m1.done = hasDiscord;

      // Mission 3: Forge a trophy → done if readyTrophy is null AND there's a forged one
      const hasForged = this.userTrophies.some(t => t.forged);
      const m3 = this.missions.find(m => m.id === 3);
      if (m3) m3.done = hasForged;
    },
    handleMission(mission) {
      // Route to where the mission asks the user to go
      const routes = {
        1: '/profile',  // Connect Discord → settings/integrations
        2: '/profile',  // Sync badges
        3: '/forge',    // Forge a trophy
        4: '/feed'      // Share achievement
      };
...
**CRÍTICO sobre `api` import:** El path `../api/api.js` asume estructura estándar. Claude Code debe verificar el path real con:

```bash
find resources/web/js -name "api.js" -type f
```

Y ajustar el import. También debe verificar que `api.get()` existe (vs `axios.get()` directo). Si el proyecto usa axios directo, cambiar a:

```js
import axios from 'axios';
// ... y usar axios.get('/api/badges') en vez de api.get('/badges')
```

Verificar con:

```bash
grep -n "import api\|import axios" resources/web/js/pages/Dashboard.vue resources/web/js/pages/Forge.vue 2>/dev/null | head -5
```

Usar el mismo patrón que otras páginas del proyecto.
...
.dashboard-page {
  min-width: 0;
  max-width: 100%;
  padding-top: 0;  // main-header is position:fixed with top:0, .page has padding-top:90px
}
...
Abrir `/dashboard` logueado. Debe renderizar todas las secciones sin errores de consola. Las API calls pueden fallar por auth — no crítico, el layout debe estar bien.

**Commit:** `feat: complete Dashboard redesign with hero mascot, missions, activity, platforms`

---

### Step 3 — Verificar paths de imports y APIs

Correr estas verificaciones y **ajustar el código del Step 2 si hay discrepancias**:

```bash
# 1. Path del api module
find resources/web/js -name "api.js" -type f

# 2. Usage pattern de otras pages
grep -n "import api\|import axios\|api\.get\|axios\.get" resources/web/js/pages/Forge.vue resources/web/js/pages/TrophyRoom.vue 2>/dev/null | head -20

# 3. Campos reales de user en store
grep -n "userUsername\|username\|balances" resources/web/js/store/store.js | head -10

# 4. Shape de response de /api/forge (ver algún uso existente)
grep -rn "badges_count\|badges_owned\|forged" resources/web/js/ | head -15
```

Ajustar en Dashboard.vue según hallazgos. Si los campos reales son `total_badges` en vez de `badges_count`, etc., **adaptar los accesores**.

**Commit (si hubo ajustes):** `fix: align Dashboard API access with real backend shape`
...
Si hay errores, resolverlos antes de commitear. Casos comunes:
- Import path del `api` mal → ajustar según Step 3.
- `import.meta.url` no resuelve → usar import estático (ya está en el brief, el sprite del hero usa import estático).

Deploy:

```bash
cd ~/Documents/trophyroom && git add -A && git commit -m "feat: Phase 9D — Dashboard redesign complete" && git push origin main && ssh -i ~/.ssh/id_ed25519_do_control root@164.92.83.95 "cd /var/www/ambar && git fetch origin main && git reset --hard origin/main && npm run build && chown -R www-data:www-data storage bootstrap/cache public/build && chmod -R 775 storage bootstrap/cache && rm -rf storage/framework/views/*"
```
...
Abrir `https://app.ambar.gg/dashboard` con hard refresh (Cmd+Shift+R) + DevTools **cerrado o dock-bottom** (no dock-right).

### Hero
- [ ] Title "Welcome back, [tu username]" con el apellido en naranja glow
- [ ] Tag "System online · Player profile" arriba
- [ ] Tagline con la cantidad de badges/platforms
- [ ] 4 stats: Badges (en chartreuse con glow), Trophies, Achievements, Platforms
- [ ] 2 CTAs: "Forge trophy" (naranja) y "View virtual hall" (outline)
- [ ] Mascota T-Rex voxel 3D a la derecha, ~340×340px, con glow pulse + float animation
- [ ] Background hero con radial naranja + chartreuse

### Ready to forge (si hay trophy listo)
- [ ] Card con border chartreuse tenue, radial chartreuse de fondo
- [ ] Status "X/X badges collected — ready" con pulse dot verde
- [ ] Nombre del trophy en VT323 grande
- [ ] Description, meta (XP/Uru/Ambar cost), botón Forge now
- [ ] Badge grande del trophy a la derecha con glow

### Trophies in progress
- [ ] Grid de thumbs (3-4) con icono, nombre, badges count, progress bar
- [ ] Progress bars: vacío = gris, in progress = chartreuse, full = naranja

### Daily missions
- [ ] Terminal con header "1/4 completed" y "+Y Ambar pending"
- [ ] 4 rows con check circle, title, sub, reward pill, botón action
- [ ] Rows completados: opacity 60% + check verde + botón "Done"
- [ ] Click en action navega a la ruta correspondiente

### Recent activity
- [ ] Si hay notificaciones: lista con dot verde + text + time ago
- [ ] Si vacío: empty state con mascota + "The chest is empty" + CTA "Go to Forge"

### Connected platforms
- [ ] 5 cells: Discord/Steam/GitHub/Riot/Strava
- [ ] Synced tienen logo naranja + status "Synced · N"
- [ ] Off tienen logo gris + status "Connect"

### Terminal footer
- [ ] "trophyroom.v2.0 · build 4.7" con cursor blinkeando
- [ ] "session · Xh Ym · uptime nominal"

### Funcional
- [ ] Click "Forge trophy" va a `/forge`
- [ ] Click "View virtual hall" va a `/virtual-hall/{username}`
- [ ] Click en un trophy thumb va a `/forge?trophy={id}`
- [ ] Click en platform cell va a `/profile`
- [ ] Console sin errores nuevos

### Mobile 375px
- [ ] Hero mascota más chica (220px), stats y CTAs se apilan
- [ ] Trophy hero: columna única, badge más chico
- [ ] Activity empty: mascota arriba, texto abajo
- [ ] Platforms strip se adapta al ancho
...
2. Confirmación hashes local == server
3. Ajustes hechos en Step 3 (paths, API shapes, field names reales)
4. Captures de desktop `/dashboard` + mobile 375px
5. Warnings/errores nuevos en consola

---

## Rollback plan

Si algo rompe catastróficamente:
...
## Backlog después de 9D

- **Missions reales:** Wire a backend cuando exista endpoint de daily missions. Por ahora es hardcoded con state derivado del real data disponible.
- **Platform "connect" flow:** El click actualmente va a `/profile`. Debería disparar directamente el OAuth flow cuando esté listo.
- **Activity timeline granular:** Hoy muestra notifications flat. Podría agruparse por día o filtrar por tipo.
- **Trophy hero fallback:** Si no hay `readyTrophy`, la sección no se muestra. Podríamos mostrar un "Keep forging — X badges away" CTA en su lugar.
